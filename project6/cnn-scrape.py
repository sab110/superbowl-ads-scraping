...
def safe_filename(name: str, max_length: int = 100) -> str:
    ...
    safe = safe.rstrip(" .,")
    # Allow spaces instead of underscores for readability
    safe = safe.replace("_", " ")
...
# -------------------
AWARDS = {
    "CNN": "https://www.unblock.coffee/cnns/?adsyear={year}",
    # Add others back when ready...
}
...
for award_name, base_pattern in AWARDS.items():
    ...
    for year in range(2025, 2011, -1):
        ...
        for category_url in category_links:
            # Replace hyphens with underscores in folder names
            # category_name = category_url.split("/")[-1].split("?")[0].replace("-", "_")
            category_name = category_url.split("/")[-1].split("?")[0].replace("-", " ")
            ...
            for idx, item in enumerate(project_items, start=1):
                ...
                # ---- Credits fix ----
                credits = []
                # Case 1: Normal table with role + name
                ...
                # Case 2: Sections with heading + only creative-name
